package com.bftv.fui.net.adapter;

import android.graphics.Bitmap;

import com.bftv.fui.net.OkGo;
import com.bftv.fui.net.cache.CacheEntity;
import com.bftv.fui.net.cache.CacheManager;
import com.bftv.fui.net.cache.CacheMode;
import com.bftv.fui.net.callback.AbsCallback;
import com.bftv.fui.net.callback.AbsCallbackWrapper;
import com.bftv.fui.net.exception.OkGoException;
import com.bftv.fui.net.model.HttpHeaders;
import com.bftv.fui.net.model.Response;
import com.bftv.fui.net.okhttp.okhttp3.Callback;
import com.bftv.fui.net.request.BaseRequest;
import com.bftv.fui.net.utils.HeaderParser;
import com.bftv.fui.net.utils.HttpUtils;

import java.io.IOException;
import java.net.SocketTimeoutException;

import com.bftv.fui.net.okhttp.okhttp3.Headers;
 ...
 * ================================================
 */
public class CacheCall<T> implements com.bftv.fui.net.adapter.Call<T> {

    private volatile boolean canceled;
    private boolean executed;
    private BaseRequest baseRequest;
    private com.bftv.fui.net.okhttp.okhttp3.Call rawCall;
    ...
    public void execute(AbsCallback<T> callback) {
        ...
        rawCall.enqueue(new Callback() {
            @Override
            public void onFailure(com.bftv.fui.net.okhttp.okhttp3.Call call, IOException e) {
                if (e instanceof SocketTimeoutException && currentRetryCount < baseRequest.getRetryCount()) {
                    //超时重试处理
                    currentRetryCount++;
                    com.bftv.fui.net.okhttp.okhttp3.Call newCall = baseRequest.generateCall(call.request());
            ...
            @Override
            public void onResponse(com.bftv.fui.net.okhttp.okhttp3.Call call, com.bftv.fui.net.okhttp.okhttp3.Response response) throws IOException {
    ...
    /** 失败回调，发送到主线程 */
    @SuppressWarnings("unchecked")
    private void sendFailResultCallback(final boolean isFromCache, final com.bftv.fui.net.okhttp.okhttp3.Call call, final com.bftv.fui.net.okhttp.okhttp3.Response response, final Exception e) {
        final CacheMode cacheMode = baseRequest.getCacheMode();
    ...
    /** 成功回调，发送到主线程 */
    private void sendSuccessResultCallback(final boolean isFromCache, final T t, final com.bftv.fui.net.okhttp.okhttp3.Call call, final com.bftv.fui.net.okhttp.okhttp3.Response response) {
        final CacheMode cacheMode = baseRequest.getCacheMode();
    ...
    public Response<T> execute() throws Exception {
        ...
        com.bftv.fui.net.okhttp.okhttp3.Call call = baseRequest.getCall();
        if (canceled) {
            call.cancel();
        }
    ...
    private Response<T> parseResponse(com.bftv.fui.net.okhttp.okhttp3.Response rawResponse) throws Exception {
        //noinspection unchecked
    ...
    @Override
    public com.bftv.fui.net.adapter.Call<T> clone() {
